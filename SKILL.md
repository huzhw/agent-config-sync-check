...
14. **chrome-devtools 目录唯一（McpUserDataDirUnique）**：chrome-devtools 是四端共用的浏览器 MCP，但 Chromium 同一 profile 目录**只允许一个活动实例**——两端 `--userDataDir` 相同 = 谁后拉起浏览器谁秒死（`Target closed`）且不自愈，直到该端重启。检查解析四端（含 `.claude.json`）chrome-devtools 的 userDataDir，重复报 `McpUserDataDirDuplicate`（**只报告不自动修**，目录分配登记在 `sync-config.json` 的 `perEndArgs`）；无 userDataDir 报 `McpUserDataDirAbsent`（WARN，isolated/headless 属有意时忽略）。2026-09-12 实战：CC/Codex/ZCode/DSH 四端共用同一个 `User Data MCP` 目录，DSH 浏览器长期被先到者顶死；同日 DSH 引擎升级 0.12.2 后又暴露 `command: npx` 直接 spawn 不出（引擎起不了 `.cmd` 垫片），故 DSH 端规范形态 = `node` + `npx-cli.js` 前置 + 独占目录 `User Data MCP-dsh`，其余端各用 `User Data MCP-<端名>`
