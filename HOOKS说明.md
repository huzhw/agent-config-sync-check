...
> Claude 系走 settings.json 挂 shell 脚本；DSH 走声明式规则 + 插件（无用户 hooks 机制）；Codex 走 config.toml 的 hooks 特性。
...
| 任务完成通知 | tokentracker（Stop/SessionEnd） | `dsh-notification` 插件（桌面通知） | tokentracker（`notify` 配置） | 未接 |
...
| Zcode | settings.json hooks 段（Claude 同构） | `~\.zcode\settings.json` | 引用 `~\.claude\hooks\*.sh` 同一路径 |
| Codex | config.toml `[[hooks.*]]` + `[features] hooks = true` | `~\.codex\config.toml`（每条 hook 有 trusted_hash，命令变更需重新信任） | Bash 拦截引 `~\.claude\hooks\`；记忆守卫用 `~\.codex\hooks\*codex.sh` 适配版 |
...
2. **改 Codex 记忆守卫** → 只改 `~\.codex\hooks\*codex.sh`（不入 git）
3. **改 DSH 规则** → 只改 `~\.dsh\rules.yaml`（热重载即生效；跑 `/rules list` 验证），不碰内置基线文件（插件升级会被覆盖，差量一律写用户级 rules.yaml）
4. **新增防护能力** → 四端各走各路：脚本类进 coding-rules\hooks 分发，DSH 类进 rules.yaml，Codex 专属适配进 ~/.codex\hooks\
